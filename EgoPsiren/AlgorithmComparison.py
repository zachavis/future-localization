...
PRINT_DEBUG_IMAGES_FINAL = False
...
if __name__ == "__main__":
    ...
    #overfit_imploc_grad_192_miniset.pt
    #overfit_imploc_aligned_goal_192.pt



    #overfit_imploc_192_full_final
    #VAE_192_imageprior_full


    #'overfit_imploc_192_full_final.pt'
    #'overfit_VAE_192_imageprior_full.pt'
    ...
    #Full
    #overfit_imploc_192_full_final_grad.pt
    #overfit_VAE_192_imageprior_full.pt

    #NonEgo Synth
    #overfit_imploc_192_full_synthetic_NonEgo_grad.pt


    #No Reg Synth
    #overfit_imploc_192_full_synth_grad_NOREG.pt

    #siren only
    #overfit_imploc_192_full_synth_SIREN.pt

    # full network no ego
    # overfit_imploc_192_NonEgo_full_grad.pt
    
    #full network no reg
    # overfit_imploc_192_full_final_NOREG
    #full network only siren
    # overfit_imploc_192_full_final_SIREN

    network = torch.load('overfit_imploc_192_full_final_SIREN.pt') #torch.load('hypernet_1200imgs_300epochs.pt') #overfit_test_network_exp_newnewloss
    ...
    partial_folder_path = 'S:\\fut_loc\\test\\' #'S:\\synth_marketplace_random2020\\test\\' #'S:\\fut_loc\\synth\\' #'S:\\fut_loc\\test\\' #20150401_walk_00\\' #'S:\\synth_marketplace_trials2021\\test\\' #'S:\\nav_trials\\trials\\btest\\' # #'S:\\nav_trials\\trials\\atest\\' #
    
    folder_name = '20150402_grocery' #'20150418_mall_00' #'20150401_walk_00' #'20150419_ikea' # 'definitelynotzach8002_t36_p12' #'acofre20167850_t38_p18' #'10000000_test_00' #'marketplace6203_trand_p0' #
    ...
    trajectory_counter = 0
     ...
     #'knn_alexfeats_aligned.knn'
     #'knn_alexfeats_aligned_synth.knn'
     #knn_alexfeats_aligned_full_FIXED
     #knn_alexfeats_aligned_synth_FIXED
    knnPickle = open('knn_alexfeats_aligned_full_FIXED.knn','rb')
    ...
    dictPickle = open('knn_traintraj_aligned_full_FIXED.dict','rb')
    ...
    for folder_name in next(os.walk(partial_folder_path))[1]:
        ...
        for iFrame in range(frameOffset,frameEnd):
            ...
            if (len(tr['XYZ'][1]) < 2):
                ...
                print('FROM FOLDER', folder_name, 'GETTING FRAME',iFrame,'/',frameEnd )
            ...
            if USE_EGO:
                ...
                img_resized_alexnet = cv2.resize(img_rectified, (int(256), int(256)))
                img_channel_swap_AlexNet = np.moveaxis(img_resized_alexnet,-1,0).astype(np.float32)
            ...
            # FINALLY this trajectory has made it to error computation
            image_names.append( vFilename[iFrame] )
        ...
        #for frame in RESIZED_IMAGE_DICTIONARY.keys():
            ...
            test_coord_value = DataGens.Coords2ValueFastWS_NEURIPS(all_pixel_coords_xformed,{0:test_ws_trajectory},None,None,stddev=1)#.5)
            ...
            if not USE_INTENSITY:
                ...
                image_siren_image = (image_siren_predictions['siren_out'], image_siren_predictions['model_in'])
            ...
            ########################################################################
            # Let's plan a path through the image via greedy descent
            ########################################################################
            ...
            #traj_t_pix = smoothed_min_along_y[below_thresh==True]
            ...
            traj_x, traj_y = DataReader.Polar2Coord(traj_t,traj_r)



            # THIS IS OUR FINAL TRAJECTORY FOR COMPARISON!
            traj_t_pix_resampled, traj_r_pix_resampled = DataGens.InterpAlongLine(traj_t_pix,traj_r_pix,25)
            traj_x_resampled, traj_y_resampled = DataGens.InterpAlongLine(traj_x,traj_y,25)
            ...
            x_AlexNet, y_AlexNet = DataReader.Polar2Coord(t_AlexNet,np.exp(logr_AlexNet))
            
            
            # THIS IS OUR FINAL TRAJECTORY FOR COMPARISON!
            knn_t_pix_resampled, knn_r_pix_resampled = DataGens.InterpAlongLine(tpix_AlexNet,rpix_AlexNet,25)
            knn_x_resampled, knn_y_resampled = DataGens.InterpAlongLine(x_AlexNet,y_AlexNet,25)
            ...
            x_VAE, y_VAE = DataReader.Polar2Coord(traj_vae_t,np.exp(traj_vae_logr))


            # THIS IS OUR FINAL TRAJECTORY FOR COMPARISON!
            vae_t_pix_resampled, vae_r_pix_resampled = DataGens.InterpAlongLine(traj_vae_tpix,traj_vae_logrpix,25)
            vae_x_resampled, vae_y_resampled = DataGens.InterpAlongLine(x_VAE,y_VAE,25)


            
            # THIS IS OUR GROUND TRUTH TRAJECTORY FOR COMPARISON!
            trajnp = np.array(test_pix_trajectory)
            traj_lengths.append(len(trajnp))
            

            GT_t_pix_resampled, GT_r_pix_resampled = DataGens.InterpAlongLine(trajnp[:,0],trajnp[:,1],25)
            
            gt_t = ego_pix2t(trajnp[:,0])
            gt_logr = ego_pix2r(trajnp[:,1])

            x_GT, y_GT = DataReader.Polar2Coord(gt_t,np.exp(gt_logr))
            GT_array = np.zeros((len(x_GT),2))
            GT_array[:,0] = x_GT
            GT_array[:,1] = y_GT
            
            trajnp_ws = np.array(test_ws_trajectory)
            

            avg_speeds.append(np.mean(np.linalg.norm(trajnp_ws[1:]-trajnp_ws[:-1],axis=1)))

            GT_x_resampled, GT_y_resampled = DataGens.InterpAlongLine(x_GT,y_GT,25)
            



            OUR_METHOD = np.zeros((len(traj_t_pix_resampled),2))
            OUR_METHOD[:,0] = traj_t_pix_resampled
            OUR_METHOD[:,1] = traj_r_pix_resampled

            OUR_METHOD_WS = np.zeros((len(traj_x_resampled),2))
            OUR_METHOD_WS[:,0] = traj_x_resampled
            OUR_METHOD_WS[:,1] = traj_y_resampled
            

            
            start_coords_array = np.ones((25,2))
            start_coords_array[:,0] *= ws_coords[0,0]
            start_coords_array[:,1] *= ws_coords[0,1]

            
            orientation_array = np.ones((25,1))
            orientation_array[:,0] *= frames[0]

            #np.savetxt('test.txt'.format(trajectory_counter), np.vstack((OUR_METHOD_WS,ws_coords[:25]))*0.507, delimiter=',')
            #np.savetxt('traj\\imploc\\imploc_traj{:04d}.txt'.format(trajectory_counter), np.hstack((OUR_METHOD_WS*0.507,start_coords_array*0.507,orientation_array)), delimiter=',')

            
            KNN_METHOD = np.zeros((len(traj_t_pix_resampled),2))
            KNN_METHOD[:,0] = knn_t_pix_resampled
            KNN_METHOD[:,1] = knn_r_pix_resampled
            
            KNN_METHOD_WS = np.zeros((len(knn_x_resampled),2))
            KNN_METHOD_WS[:,0] = knn_x_resampled
            KNN_METHOD_WS[:,1] = knn_y_resampled
            

            #np.savetxt('traj\\knn\\knn_traj{:04d}.txt'.format(trajectory_counter), np.hstack((KNN_METHOD_WS*0.507,start_coords_array*0.507,orientation_array)), delimiter=',')

            
            VAE_METHOD = np.zeros((len(traj_t_pix_resampled),2))
            VAE_METHOD[:,0] = vae_t_pix_resampled
            VAE_METHOD[:,1] = vae_r_pix_resampled

            VAE_METHOD_WS = np.zeros((len(vae_x_resampled),2))
            VAE_METHOD_WS[:,0] = vae_x_resampled
            VAE_METHOD_WS[:,1] = vae_y_resampled
            
            #np.savetxt('traj\\vae\\vae_traj{:04d}.txt'.format(trajectory_counter), np.hstack((VAE_METHOD_WS*0.507,start_coords_array*0.507,orientation_array)), delimiter=',')

            
            GT_METHOD = np.zeros((len(traj_t_pix_resampled),2))
            GT_METHOD[:,0] = GT_t_pix_resampled
            GT_METHOD[:,1] = GT_r_pix_resampled

            GT_METHOD_WS = np.zeros((len(GT_x_resampled),2))
            GT_METHOD_WS[:,0] = GT_x_resampled
            GT_METHOD_WS[:,1] = GT_y_resampled
            
            #np.savetxt('traj\\gt\\gt_traj{:04d}.txt'.format(trajectory_counter), np.hstack((GT_METHOD_WS*0.507,start_coords_array*0.507,orientation_array)), delimiter=',')

            
            OUR_METHOD_DIFF = OUR_METHOD - GT_METHOD
            OUR_METHOD_DIFF_NORM = np.linalg.norm(OUR_METHOD_DIFF,axis=1)
            OUR_METHOD_ERROR = np.mean(OUR_METHOD_DIFF_NORM)
            IMPLOC_ERROR.append(OUR_METHOD_ERROR)

            OUR_METHOD_DIFF_WS = OUR_METHOD_WS - GT_METHOD_WS
            OUR_METHOD_DIFF_NORM_WS = np.linalg.norm(OUR_METHOD_DIFF_WS,axis=1)
            OUR_METHOD_ERROR_WS = np.mean(OUR_METHOD_DIFF_NORM_WS)
            IMPLOC_ERROR_WS.append(OUR_METHOD_ERROR_WS)


            KNN_METHOD_DIFF = KNN_METHOD - GT_METHOD
            KNN_METHOD_DIFF_NORM = np.linalg.norm(KNN_METHOD_DIFF,axis=1)
            KNN_METHOD_ERROR = np.mean(KNN_METHOD_DIFF_NORM)
            KNN1_ERROR.append(KNN_METHOD_ERROR)

            KNN_METHOD_DIFF_WS = KNN_METHOD_WS - GT_METHOD_WS
            KNN_METHOD_DIFF_NORM_WS = np.linalg.norm(KNN_METHOD_DIFF_WS,axis=1)
            KNN_METHOD_ERROR_WS = np.mean(KNN_METHOD_DIFF_NORM_WS)
            KNN1_ERROR_WS.append(KNN_METHOD_ERROR_WS)


            VAE_METHOD_DIFF = VAE_METHOD - GT_METHOD
            VAE_METHOD_DIFF_NORM = np.linalg.norm(VAE_METHOD_DIFF,axis=1)
            VAE_METHOD_ERROR = np.mean(VAE_METHOD_DIFF_NORM)
            VAE_ERROR.append(VAE_METHOD_ERROR)

            VAE_METHOD_DIFF_WS = VAE_METHOD_WS - GT_METHOD_WS
            VAE_METHOD_DIFF_NORM_WS = np.linalg.norm(VAE_METHOD_DIFF_WS,axis=1)
            VAE_METHOD_ERROR_WS = np.mean(VAE_METHOD_DIFF_NORM_WS)
            VAE_ERROR_WS.append(VAE_METHOD_ERROR_WS)

            trajectory_counter+=1

            print('error appended.')


            if PRINT_DEBUG_IMAGES_FINAL:
                ...
                axes[0+load_offset].plot(traj_t_pix_resampled,traj_r_pix_resampled,'cx')
...
our_errors = np.array(IMPLOC_ERROR)
our_errors_average = np.mean(our_errors)
our_errors_stddev = np.std(our_errors)
our_errors_best = np.min(our_errors)
our_errors_best_idx = np.argmin(our_errors)
print('Our method:')
print('\tAverage error:', our_errors_average)
print('\t\tstddev:', our_errors_stddev)
print('\tBest error:', our_errors_best)
print('\t\ton image:', image_names[our_errors_best_idx], 'in folder',folder_names[our_errors_best_idx])
print('')

knn_errors = np.array(KNN1_ERROR)
knn_errors_average = np.mean(knn_errors)
knn_errors_stddev = np.std(knn_errors)
knn_errors_best = np.min(knn_errors)
knn_errors_best_idx = np.argmin(knn_errors)
print('KNN1 method:')
print('\tAverage error:', knn_errors_average)
print('\t\tstddev:', knn_errors_stddev)
print('\tBest error:', knn_errors_best)
print('\t\ton image:', image_names[knn_errors_best_idx], 'in folder',folder_names[knn_errors_best_idx])
print('')



vae_errors = np.array(VAE_ERROR)
vae_errors_average = np.mean(vae_errors)
vae_errors_stddev = np.std(vae_errors)
vae_errors_best = np.min(vae_errors)
vae_errors_best_idx = np.argmin(vae_errors)
print('VAE method:')
print('\tAverage error:', vae_errors_average)
print('\t\tstddev:', vae_errors_stddev)
print('\tBest error:', vae_errors_best)
print('\t\ton image:', image_names[vae_errors_best_idx], 'in folder',folder_names[vae_errors_best_idx])
print('')
...
our_errors_WS = np.array(IMPLOC_ERROR_WS)
our_errors_average_WS = np.mean(our_errors_WS)
our_errors_stddev_WS = np.std(our_errors_WS)
our_errors_best_WS = np.min(our_errors_WS)
our_errors_best_idx_WS = np.argmin(our_errors_WS)
print('Our method World Space:')
print('\tAverage error:', our_errors_average_WS)
print('\t\tstddev:', our_errors_stddev_WS)
print('\tBest error:', our_errors_best_WS)
print('\t\ton image:', image_names[our_errors_best_idx_WS], 'in folder',folder_names[our_errors_best_idx_WS])
print('')

knn_errors_WS = np.array(KNN1_ERROR_WS)
knn_errors_average_WS = np.mean(knn_errors_WS)
knn_errors_stddev_WS = np.std(knn_errors_WS)
knn_errors_best_WS = np.min(knn_errors_WS)
knn_errors_best_idx_WS = np.argmin(knn_errors_WS)
print('KNN1 method World Space:')
print('\tAverage error:', knn_errors_average_WS)
print('\t\tstddev:', knn_errors_stddev_WS)
print('\tBest error:', knn_errors_best_WS)
print('\t\ton image:', image_names[knn_errors_best_idx_WS], 'in folder',folder_names[knn_errors_best_idx_WS])
print('')



vae_errors_WS = np.array(VAE_ERROR_WS)
vae_errors_average_WS = np.mean(vae_errors_WS)
vae_errors_stddev_WS = np.std(vae_errors_WS)
vae_errors_best_WS = np.min(vae_errors_WS)
vae_errors_best_idx_WS = np.argmin(vae_errors_WS)
print('VAE method World Space:')
print('\tAverage error:', vae_errors_average_WS)
print('\t\tstddev:', vae_errors_stddev_WS)
print('\tBest error:', vae_errors_best_WS)
print('\t\ton image:', image_names[vae_errors_best_idx_WS], 'in folder',folder_names[vae_errors_best_idx_WS])
print('')

avg_avg_speeds =np.mean(np.array(avg_speeds))
print('Average speed is:',avg_avg_speeds)
print('')

avg_traj_length = np.mean(np.array(traj_lengths))
print('Average Traj Length:',avg_traj_length, 'over',len(traj_lengths),'trajectories')
print('')


print("Finished on",len(IMPLOC_ERROR),"images.")